...
export class BreakingChangesPanel {
  ...
  public static createOrShow(extensionUri: vscode.Uri) {
    ...
    BreakingChangesPanel.currentPanel = new BreakingChangesPanel(
      panel,
      extensionUri
    );
  }
  ...
  private async analyzeBreakingChanges(
    ...
    try {
      ...
      const result = await this._breakingChangeService.analyzeBreakingChanges(
        sourceBranch,
        targetBranch
      );
      ...
      this._logger.logError(
        `Error analyzing breaking changes: ${error}`,
        error,
        "BreakingChangesPanel"
      );
      this._panel.webview.postMessage({
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    // Check if we're in development mode by looking for source maps in the bundle
    ...
    <head>
      ...
      <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src ${scriptSrc}; img-src ${webview.cspSource} https:; connect-src https:; font-src ${webview.cspSource};">
